...
def print_death_age(people_dict):
    ...
    for x in people_dict:
        death_year = people_dict[x][DEATH_YEAR_INDEX]
        birth_year = people_dict[x][BIRTH_YEAR_INDEX]
        death_age = death_year - birth_year
        name = people_dict[x][NAME_INDEX]
...
def count_genders(people_dict):
    ...
    for gender in people_dict:
        ...
        if people_dict[gender][GENDER_INDEX] == 'M':
            m += 1
        else:
            f += 1
    print()
    print('Number of males and females in dicitonary')
    print(f'Males: {m}')
    print(f'Females: {f}')
...
def print_marriages(marriages_dict, people_dict):
    ...
    for x in marriages_dict:
        ...
        husband_key = marriages_dict[x][HUSBAND_KEY_INDEX]
        man = people_dict[husband_key][NAME_INDEX]
        man_wedding_age = marriages_dict[x][WEDDING_YEAR_INDEX] - people_dict[husband_key][BIRTH_YEAR_INDEX]
        # wife_key = marriages_dict[x][WIFE_KEY_INDEX]
        # wedding_year = marriages_dict[x][WEDDING_YEAR_INDEX]    
        
        wedding_year = marriages_dict[x][WEDDING_YEAR_INDEX]

        wife_key = marriages_dict[x][WIFE_KEY_INDEX]
        woman = people_dict[wife_key][NAME_INDEX]
        wife_wedding_age = marriages_dict[x][WEDDING_YEAR_INDEX] - people_dict[wife_key][BIRTH_YEAR_INDEX]


        print(f" {man} {man_wedding_age} > {wedding_year} < {woman} {wife_wedding_age} ")
    ...
    #     if husband_key == people_dict[key]:
    #         husband_name = husband_key[key][NAME_INDEX]
    #         husband_birthday = husband_key[key][BIRTH_YEAR_INDEX]
    #     elif wife_key == people_dict[key]:    
    #         wife_name = wife_key[key][NAME_INDEX]
    #         wife_birthday = wife_key[key][BIRTH_YEAR_INDEX]
                    
            
    #     wife_wedding_age = wedding_year - wife_birthday
    #     husband_wedding_age = wedding_year - husband_birthday
