def main():
    ...
    molecule_table = make_molecule_table()
    # mole = periodic_table[]
    ...
    element_quantity = parse_formula(chemical_formula, periodic_table)
    compound_name = get_formula_name(chemical_formula, molecule_table)
    # Call the compute_molar_mass function to compute the
    # molar mass of the molecule from the compound list.
    molar_mass = compute_molar_mass(element_quantity, periodic_table)
...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    for x in symbol_quantity_list:
        ...
        symbol_list = x[SYMBOL_INDEX]
        quantity_list =x [QUANTITY_INDEX]

        atomic_mass = periodic_table_dict[symbol_list][ATOMIC_MASS_INDEX]
        # Get the atomic mass for the symbol from the dictionary.
        # Multiply the atomic mass by the quantity.
        
        total_molar_mass += atomic_mass * quantity_list
        # Add the product into the total molar mass.

    # Return the total molar mass.
    return total_molar_mass
def make_molecule_table():
